  ...
  const getSummary = async () => {
    if (!summarizedRef.current) {
      ...
      if (response.ok) {
        ...
        formattedResponse = formattedResponse.replace(/\*([\s\S]+?)(?:<\/br>|$)/g, '<ul class="mb-0 list-dot" style="padding-left: 20px;"><li>$1</li></ul>');
        formattedResponse = formattedResponse.replace(/\+([\s\S]+?)(<\/br>|$)/g, '<ul class="mb-0 list-circle" style="padding-left: 36px;"><li>$1</li></ul>');
